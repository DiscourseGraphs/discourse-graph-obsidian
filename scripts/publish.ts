...
async function createRelease(
  ...
  try {
    ...
    const assetsToUpload = [
      ...
      { filename: "styles.css", required: false }, // Optional
    ];
    ...
    for (const asset of assetsToUpload) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        const contentType = asset.filename.endsWith(".js")
        ...
        try {
          ...
          if (asset.required) {
            throw new Error(
              `Failed to upload required asset: ${asset.filename}`,
            );
          }
        }
      } else if (asset.required) {
        throw new Error(`Required asset not found: ${asset.filename}`);
      } else {
        console.log(
          `  ⚠️  Optional asset ${asset.filename} not found, skipping`,
        );
      }
    }

    console.log(`✓ All assets uploaded to release`);
  } catch (error: any) {
